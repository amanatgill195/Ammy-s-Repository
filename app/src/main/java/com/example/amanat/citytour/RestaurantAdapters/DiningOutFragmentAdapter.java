...
public class DiningOutFragmentAdapter extends RecyclerView.Adapter<RestaurantViewHolder> {
    private ArrayList<RestaurantModel> restaurantModelArrayList;
    private DiningOutFragment diningOutFragment;
    private Double latDestination, lonDestination;
    private GpsTracker gpsTracker;
    private String locationDistance;
    private String deliver = "Don't Deliver", collection = "diningOutRestaurants";

    public DiningOutFragmentAdapter(ArrayList<RestaurantModel> restaurantModelArrayList, DiningOutFragment diningOutFragment) {
        this.restaurantModelArrayList = restaurantModelArrayList;
        this.diningOutFragment = diningOutFragment;
    }
    ...
    public void onBindViewHolder(@NonNull RestaurantViewHolder holder, final int position) {
        ...
        try {
            if (ContextCompat.checkSelfPermission(diningOutFragment.getActivity(), android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(diningOutFragment.getActivity(), new String[]{android.Manifest.permission.ACCESS_FINE_LOCATION}, 101);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

        gpsTracker = new GpsTracker(diningOutFragment.getActivity());
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(diningOutFragment.getActivity(), RestaurantDetail.class);
                ...
                bundle.getString("rating", String.valueOf(restaurantModelArrayList.get(position).getRatings()));
